...
describe("password hashing", () => {
  test("hashes and verifies passwords", async () => {
    ...
    await expect(verifyPassword("correct-password", hashedPassword)).resolves.toEqual({
      valid: true,
      needsRehash: false,
    });
    await expect(verifyPassword("wrong-password", hashedPassword)).resolves.toEqual({
      valid: false,
      needsRehash: false,
    });
  });
  ...
  test("accepts legacy plaintext passwords and marks them for rehash", async () => {
    await expect(verifyPassword("legacy-password", "legacy-password")).resolves.toEqual({
      valid: true,
      needsRehash: true,
    });
  });
